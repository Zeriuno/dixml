...
<form action ="" method="get" style="padding: 5px; border: 1px solid black; width: 480px;">
    ...
    <table>
        ...
        <td> <input type="hidden" name="run" value="testparam.xq" /></td>
...
<h3><u>Mot</u> : {$mot}</h3>
{
let $fichier := "test.html"
for $entree in doc($fichier)//ul[@class="vedette"]
where $entree/mot/text() = $mot
return
  <div>
  <h3><u>Catégorie grammaticale</u> : {$entree/categorie/cat}</h3>
  <table>
    <tr>
    ...
    {
      for $traduction in $entree/traduc/ul[@class="contenu"]
      return
